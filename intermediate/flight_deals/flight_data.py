...
def find_cheapest_flight(data):
    ...
    cheapest_flight = get_flight_data(first_flight, nr_stops, lowest_price)

    for flight in data["data"]:
        price = float(flight["price"]["grandTotal"])
        if price < lowest_price:
            lowest_price = price
            cheapest_flight = get_flight_data(flight, nr_stops, lowest_price)
            print(f"Lowest price to {cheapest_flight.destination_airport} is EUR {lowest_price}")

    return cheapest_flight

def get_flight_data(flight, nr_stops, lowest_price):
